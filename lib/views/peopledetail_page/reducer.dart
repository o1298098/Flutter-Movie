...
PeopleDetailPageState _onInit(PeopleDetailPageState state, Action action) {
  final PeopleDetailModel m=action.payload??PeopleDetailModel.fromParams(also_known_as: List<String>());
  final PeopleDetailPageState newState = state.clone();
  newState.peopleDetailModel=m;
  double s=(m.biography.length*Adapt.px(30)/(Adapt.screenW()/Adapt.onepx()-Adapt.px(60))).floorToDouble();
  newState.biographyHeight=s*Adapt.px(30);
  return newState;
}
PeopleDetailPageState _onSetCreditModel(PeopleDetailPageState state, Action action) {
  CombinedCreditsModel m=action.payload??CombinedCreditsModel.fromParams(cast: List<CastData>(),crew:List<CrewData>());
  final PeopleDetailPageState newState = state.clone();
  newState.creditsModel=m;
  return newState;
}
PeopleDetailPageState _onShowBiography(PeopleDetailPageState state, Action action) {
  final PeopleDetailPageState newState = state.clone();
