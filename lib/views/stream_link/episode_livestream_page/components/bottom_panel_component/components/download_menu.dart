...
class DownloadMenu extends StatelessWidget {
  ...
  final Function(TvShowStreamLink) playVideo;
  const DownloadMenu({this.links, this.tvName, this.playVideo});
  @override
...
class _DownLoadTaskList extends StatefulWidget {
  final String tvName;
  final Function(TvShowStreamLink) onPlayVideo;
  final List<TvShowStreamLink> list;
  const _DownLoadTaskList({Key key, this.list, this.tvName, this.onPlayVideo})
      : super(key: key);
  @override
...
class _DownLoadTaskListState extends State<_DownLoadTaskList> {
  ...
  Future _createTask(TvShowStreamLink link) async {
    try {
      final _taskName =
          '${widget.tvName}_S${link.season}E${link.episode}_${link.sid}';
      final _localPath =
  ...
  void _playLoaclFile(TvShowStreamLink link) {
    final _taskName =
        '${widget.tvName}_S${link.season}E${link.episode}_${link.sid}';
  ...
  Widget build(BuildContext context) {
    return ListView.separated(
      ...
      itemBuilder: (_, index) {
        ...
        final _taskName =
            '${widget.tvName}_S${_link.season}E${_link.episode}_${_link.sid}';
        final _task = _queue?.firstWhere((e) => e.filename.contains(_taskName),
            orElse: () => null);
...
class _DownLoadCell extends StatelessWidget {
  ...
  final Function(TvShowStreamLink) downloadTap;
  final Function(TvShowStreamLink) openFileTap;
